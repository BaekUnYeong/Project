...
/*		1.영화등록 및 삭제
	...
	public void choiceDate(MovieVO mv) {
		...
		System.out.print("날짜를 선택해주세요 (최대 3일 이전)\r"
				+ "1. " + today + " 2. " + tomorrow + " 3. " + tomorrow1 + " 9. 뒤로가기" + " >");
		date = s.nextLine();
...
/*		for(int i = 0; i < showDBLength; i++){
	...
	for(int i = 0; i < seat.length; i++){
		if(i % 9 == 0){
			System.out.println();
			char tmp = (char)((i / 9) + 72); // H I J K L
...
//		}
		...
		System.out.println("예약 좌석: " + reserveSeat);
		...
		if(isReserved.equals("Y") || isReserved.equals("y")) {
			...
			int i = 0;
			for(i = 0; i < reserveDB.size(); i++){
				boolean a = false;
				for(int j = 0; j < reserveDB.size(); j++){
					if(i + 1== reserveDB.get(j).getReserveCode()){
						a = true;
						break;
					}
				}
			...
			rv.setReserveCode(i + 1);
			rv.setUserID(Session.loginUser.getId());
			
			int tempRow = reserveSeat.charAt(0) - 72;
			int tempCol = reserveSeat.charAt(1) - '0';
			...
			System.out.println("예매 코드 : " + (i + 1) + "\r사용자 아이디 : " + Session.loginUser.getId() + "\r좌석 코드" + seatCode + "\r상영 코드" +showingCode);
